...
function editItem(reply, message) {
    ...
    model.Item.findById(itemId)
        ...
        .exec(function(err, item) {
            ...
            let itemPage = item.title + " [edit](command:edit title)\n";
            if (item.description) {
                itemPage += item.description + " [edit](command:edit description)\n";
            }

            if (item.auction) {
                let minimumPrice = formatAmount(item.auction.minimumAmount, item.auction.displayUnit);
                itemPage += "Minimum price: " + minimumPrice + " [edit](command:minimum price)";
            }
            else {
                itemPage += "[set minimum price](command:minimum price)"
            }
