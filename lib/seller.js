        ...
        .exec(function(err, item) {
            ...
            if (item.auction.bids.length > 0) {
                ...
                itemPage += item.auction.bids
                    ...
                    .map((bid) => {
                        if (bid.contract && bid.contract.isClosed) {
                            return " - " + formatAmount(bid.amount, "bytes") + " (waiting for payment)";
                        }
    ...
    if (message.context && message.context.itemId) {
        ...
        model.Item.findById(itemId).where({
            ...
            if (item) {
                ...
                if (winnerBid && winnerBid.contract && winnerBid.contract.isClosed) {
                    return reply("You already requested a payment for this item.");
                }
                ...
                if (winnerBid && winnerBid.contract && winnerBid.contract.isCompleted) {
                    ...
                    let refundAddress = "A52JWMBCMWUHI35IUMH6GDSR4DVWQ7FA"; //TODO let buyer add refund address
                    let disputeAddress = "VJCR7RSBYNMBWFSTTVG33RPW6TJJIOCO"; // TODO hmm
