        ...
        .exec(function(err, item) {
            ...
            if (item.auction.bids.length > 0) {
                ...
                itemPage += item.auction.bids
                    ...
                    .map((bid) => " - " + formatAmount(bid.amount, "bytes") + " [accept](command:accept " + bid._id + ")")
                    .join("\n");
            }
    ...
    model.Item.findById(itemId)
        ...
        .exec(function(err, item) {
            ...
            if (item) {
                ...
                model.Account.findById(acceptedBid.bidder, function(err, bidder) {
                    if (err) {
                    ...
                    if (bidder) {
                        ...
                        item.save(function(err) {
                            ...
                            device.sendMessageToDevice(bidder.device, "text", "Congratulations, your bid was accepted. Please pay " + formatAmount(acceptedBid.amount, "bytes"), {
                                ifOk: function() {
                                    reply("Great! The bidder have been notified to make a payment of " + formatAmount(acceptedBid.amount, "bytes"));
                                },
                                ifError: function() {
