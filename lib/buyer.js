...
function setShippingAddress(reply, message) {
    ...
    findContractByItemId(account._id, itemId, function(err, contract, item) {
        ...
        if (!contract) {
            return account.repeatContext(message.context, function(err) {
                if (err) return reply("Something went wrong, please try again!");
                return reply("You cannot set shipphing address on this contract.");
            });
        }
        ...
        if (contract.isClosed) {
            return account.repeatContext(message.context, function(err) {
                if (err) return reply("Something went wrong, please try again!");
                return reply("You cannot set shipphing address on this contract. The contract is closed already.");
            });
        }
        ...
        item.save(function(err) {
            ...
            account.pushConversation(new model.Conversation({
                ...
                // TODO check if contract is really complete eg. refund address is filled
                reply("Ok, the [contract](command:show contract) is now complete, shipping address is set to '" + shippingAddress + "'. Now please wait for the seller to send you a payment request. " +
...
function setRefundAddress(reply, message) {
    if (!message.context) {
        return reply("First try [my bids](command:my bids) and choose an item.");
    }
    ...
    let account = message.account;
    let itemId = message.context.itemId;
    let refundAddress = message.args[0];
    ...
    findContractByItemId(account._id, itemId, function(err, contract, item) {
        ...
        if (!contract) {
            return account.repeatContext(message.context, function(err) {
                if (err) return reply("Something went wrong, please try again!");
                return reply("You cannot set refund address on this contract.");
            });
        }
        ...
        if (contract.isClosed) {
            return account.repeatContext(message.context, function(err) {
                if (err) return reply("Something went wrong, please try again!");
                return reply("You cannot set refund address on this contract. The contract is closed already.");
            });
        }

        contract.refundAddress = refundAddress;

        item.markModified("auction.bids");
        item.save(function(err) {
            if (err) {
                reply("Failed to save shipphing address. Please try again!");
                return console.error("Failed to save refund address for item " + itemId + ": " + err);
            }
            ...
            account.pushConversation(new model.Conversation({
                ...
                if (err) {
                    return reply("Refund address is saved, but the conversation is lost. Try [my bids](command:my bids) again.");
                }

                // TODO check if contract is really complete eg. shipping address is filled
                reply("Ok, the [contract](command:show contract) is now complete, refund address is set to '" + refundAddress + "'. Now please wait for the seller to send you a payment request. " +
                    "You can still change the refund address by [set refund address](command:set refund address)");
            });
        });
    });
...
module.exports.setRefundAddress = setRefundAddress;
