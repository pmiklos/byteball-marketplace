...
function showContract(reply, message) {
    ...
    model.Item.findById(itemId)
        .where("auction.winner").equals(account._id)
        .exec(function(err, item) {
            ...
            if (item) {
                ...
                if (mybid && mybid.contract) {
                    switch (mybid.contract.status) {
                        case 'DRAFT':
                            reply(mybid.contract.draft);
                            break;
                    ...
                    console.warn("Failed to find bid of winner " + account._id + " for item " + item._id);
                    reply("Your contract was not found.");
                }
            }
            else {
                reply("No contracts to show. Try [my bids](command:my bids)");
            }
